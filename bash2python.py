...
print('V1.3.2: Change directory for *.in files to ../tmp_ut/Schrauben/*.in for copy-package')
print('ausgabe als CSV bei 3 Lastfaellen fuer Berndt-Schrauben-Auswertung')
print('benoetigt:')
print('--> EForce.eforce muss durch UCI PRINT Befehl erstellt sein')
print('--> EForce_array.in mit den EID der Balken sortiert')
print('--> LF.in mit den T=? der Lastfaelle')
print('--> SET_ID_FN.in & SET_ID_FNS.in mit Set_IDs sortiert')
print('')
print('Weitere Hilfe ist im Script (auskommentiert) enthalten!!!')
print('')
print('#-----------------------------------------------------------------------------------#')
print('')
...
if len(sys.argv) < 2:
    ...
    res_source = input("\x1b[30;102mPfad + EFroce-RES File\x1b[m: ")
    inp_res = res_source
    inp_file = os.path.splitext(res_source)[0]
    inp_path = os.path.dirname(res_source)
else:
    inp_res = sys.argv[1]
    inp_file = os.path.splitext(sys.argv[1])[0]
    inp_path = os.path.dirname(sys.argv[1])

print('')
print('aktueller Pfad')
print(inp_path)
print('Ausgewaehltes RES-File:')
print(inp_res)
...
if os.path.isfile(os.path.join(inp_path, 'Schrauben', 'EForce_array.in')):
    ...
    print("ERROR: 1")
    print("")
    print("Schrauben/EForce_array.in not found.")
    print("Please insert EID of beams in --> EForce_array.in <--")
    print("Syntax for EID = 1001 1002 1003 in file EForce_array.in")
    print("1001")
    print("1002")
    print("1003")
    print("WICHTIG bitte REIHENFOLGE BEACHTEN!!!")
    print("Bitte EID der Balken in --> EForce_array.in <-- einfügen!")
    print("#--------------------------------------------------------#")
    sys.exit()
...
if os.path.isfile(os.path.join(inp_path, 'Schrauben', 'LF.in')):
    ...
    print("ERROR: 2")
    print("")
    print("Schrauben/LF.in not found.")
    print("Please insert EID of beams in --> LF.in <--")
    print("Syntax for EID = 1001 1002 1003 in file LF.in")
    print("1001")
    print("1002")
    print("1003")
    print("WICHTIG bitte REIHENFOLGE BEACHTEN!!!")
    print("Bitte EID der Balken in --> LF.in <-- einfügen!")
    print("#--------------------------------------------------------#")
    sys.exit()
...
print('')
print('Es sind', len(beam_id), 'Balken/EForce in der Auswertung vorhanden!')
print('Gesuchte Elemente suchen')
        ...
        with open(inp_res, 'r') as input_file:
            for current_line_number, line in enumerate(input_file, start=1):
                if current_line_number == line_number:
                    # Extract characters from column 26 to 54
                    extracted_data = line[25:54]
                    eforce_export_file.write(extracted_data + '\n')
            
NLast = len(test_array_lines) // len(beam_id)
print("Anzahl der Lastfaelle im Res-File", NLast)
print(len(test_array_lines))
print(len(beam_id))
print(f'print({len(test_array_lines)}/{len(beam_id)})', end=' ')
try:
    result = len(test_array_lines) / len(beam_id)
    print(f'= {result}')
    if result.is_integer():
        print("\x1b[97;41m^-- Check for integer: Yes, Result is an Integer.\x1b[m")
    else:
...
# lf_out = [int(lf) for lf in lf_out]
...
with open(os.path.join(inp_path, 'EForce_for_Excel.txt'), 'w') as output_file:
    while n < len(beam_id):
        ...
        sed_command_eid = f"sed -n '{line_number_eid}p' {inp_path}/EForce_fin.txt"
        try:
            output_eid = subprocess.check_output(sed_command_eid, shell=True, text=True, stderr=subprocess.STDOUT)
        ...
        for lf in lf_out:
            ...
            sed_command_lf = f"sed -n '{line_number_lf}p' {inp_path}/EForce_fin.txt"
            try:
                output_lf = subprocess.check_output(sed_command_lf, shell=True, text=True, stderr=subprocess.STDOUT)
                output_file.write(output_lf)
            except subprocess.CalledProcessError:
                pass  # Handle errors if 'sed' doesn't find a match
...
print('')
print('Alle temp-Daten loeschen')
print('')
...
print('#-----------------------------------------#') 
print('')
print(f'{inp_path}/EForce_NID_MON_ZUG_SCHUB.csv')
print('')
print('#-----------------------------------------#')
print('')
...
while True:
    if source.lower() == "j":
        ...
        editor_command = ['nano', f"{inp_path}/EForce_NID_MON_ZUG_SCHUB.csv"]
...
for bif_file in bif_files:
    full_path = os.path.join(os.path.dirname(inp_path), bif_file)
    print(full_path)
#%% 
# Initialize colorama
init(autoreset=True)
...
# Set the background color to green and text color to black
colored_text = f"{Back.GREEN}{Fore.BLACK}{text}{Style.RESET_ALL}"

# Prompt the user for input bif file
inp_bif = input(f"{colored_text}: ")
...
if os.path.isfile(set_id_fn_in):
    text = "FNS.in found"
    # Set the background color to green and text color to black
    col_text = f"{Back.GREEN}{Fore.BLACK}{text}{Style.RESET_ALL}"
    print(col_text)
else:
    print("#---------------------------------------------------------#")
    print("ERROR: 3")
    print("")
    print("SET_ID_FN.in or SET_ID_FNS.in not found.")
    print("Please insert SET_ID in --> SET_ID_FN.in <--")
    print("Syntax:")
    print("3001")
    print("3002")
    print("3003")
...
text_to_append = '''* * --------------- MEDINA PostProcessing 8.4.2.2 - Protocol -------------
...
user_input = input("\033[30;102mBatch-Mode starten? j/n\033[m\n")
...
while True:
    if user_input.lower() == "j":
        ...
        user_input = input("\033[30;102mInvalid input. Batch-Mode starten? j/n\033[m\n")
...
# Read user input
source = input("Continue with Batch-Mode? Enter 'j' for yes, 'n' to exit.: ")
...
while True:
    if source.lower() == "j":
        ...
        cmd = [
            "medpost84_8.4.2.2",
            "prot=" + inp_path + "/dummy.prot",
            "inprot=" + inp_path + "/FN_FNS.prot",
            "batch"
        ]
        subprocess.run(cmd)
    
        # Remove temporary files
        subprocess.run(["rm", "-rf", inp_path + "/dummy.prot", inp_path + "/TEST.temp"])
    
        # Replace dots with commas in .out files
        subprocess.run(["sed", "-i", "s/\\./,/g", inp_path + "/*.out"])
    
        # Remove lines starting with '#' in .out files
        subprocess.run(["sed", "-i", "/^#/d", inp_path + "/*.out"])
        break
...
shutil.copy(inp_path + "/FNS.out", inp_path + "/FNS_copy.temp")
shutil.copy(inp_path + "/FN.out", inp_path + "/FN_copy.temp")


# Replace ';' with '|' as the second occurrence
subprocess.run(["sed", "-i", 's/;/|/2', inp_path + "/*_copy.temp"])

# Remove everything after '|'
subprocess.run(["sed", "-i", 's/|.*//', inp_path + "/*_copy.temp"])
...
# Read the content of EForce_NID_MON_ZUG_SCHUB.csv into a list
with open(inp_path + "/EForce_NID_MON_ZUG_SCHUB.csv", 'r') as eforce_file:
    eforce = eforce_file.readlines()
...
editor_command = f"nedit {new_file_path}"
os.system(editor_command)

print('delete old files')

# Remove files with the .temp extension
...
# Remove files with the .out extension
for filename in os.listdir(inp_path):
...
# Remove files with the _start.res suffix
for filename in os.listdir(inp_path):
    if filename.endswith("_start.res"):
        file_path = os.path.join(inp_path, filename)
        if os.path.isfile(file_path):
            os.remove(file_path)
...
if os.path.exists(userparm_old_path):
    shutil.move(userparm_old_path, userparm_path)
    
print("\033[30;102mWas wurde alles verwendet:\033[m")
print(f"Eforce.eforce: {inp_res}")

for lf in lf_out:
    print(f'Lastfall T = {int(lf)}.0 ==> Lastfall RES = {int(lf) + 1}')

print(f"BIF:		{inp_bif}")
print(f"anzahl der BOFs: {bofs_num}")
