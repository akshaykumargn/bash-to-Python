...
def check_and_process_files(inp_path, inp_bif, bofs, inp_res, bofs_num):
    ...
    return set_id_fn, set_id_fns, lf

        
def write_prot_file(set_id_fn, set_id_fns, lf):
    # Auswahl Bif & Bof
...
def medpost_process(set_id_fn, set_id_fns, lf):    
    # Define the paths
...
if len(sys.argv) == 1:
    ...
    set_id_fn, set_id_fns, lf = check_and_process_files(inp_path, inp_bif, bofs, inp_res, bofs_num)
    prot_file = write_prot_file(set_id_fn, set_id_fns, lf)
    medpost_process(set_id_fn, set_id_fns, lf)
    fn_fns_create_copy()
    modify_fn_fns_semicoln()
    modify_fn_fns_split()
    write_fin_fns_fin()
    fn_fns_cleanlines()
    remove_old_files()
